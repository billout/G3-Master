import { Component, OnInit } from '@angular/core';
import { HttpHeaders, HttpResponse } from '@angular/common/http';
import { NgbModal } from '@ng-bootstrap/ng-bootstrap';

import { IEpreuves } from '../epreuves.model';

import { ITEMS_PER_PAGE } from 'app/config/pagination.constants';
import { EpreuvesService } from '../service/epreuves.service';
import { EpreuvesDeleteDialogComponent } from '../delete/epreuves-delete-dialog.component';
import { ParseLinks } from 'app/core/util/parse-links.service';

@Component({
  selector: 'jhi-epreuves',
  templateUrl: './epreuves.component.html',
})
export class EpreuvesComponent implements OnInit {
  epreuves: IEpreuves[];
  isLoading = false;
  itemsPerPage: number;
  links: { [key: string]: number };
  page: number;
  predicate: string;
  ascending: boolean;

  constructor(protected epreuvesService: EpreuvesService, protected modalService: NgbModal, protected parseLinks: ParseLinks) {
    this.epreuves = [];
    this.itemsPerPage = ITEMS_PER_PAGE;
    this.page = 0;
    this.links = {
      last: 0,
    };
    this.predicate = 'id';
    this.ascending = true;
  }

  loadAll(): void {
    this.isLoading = true;

    this.epreuvesService
      .query({
        page: this.page,
        size: this.itemsPerPage,
        sort: this.sort(),
      })
      .subscribe(
        (res: HttpResponse<IEpreuves[]>) => {
          this.isLoading = false;
          this.paginateEpreuves(res.body, res.headers);
        },
        () => {
          this.isLoading = false;
        }
      );
  }

  reset(): void {
    this.page = 0;
    this.epreuves = [];
    this.loadAll();
  }

  loadPage(page: number): void {
    this.page = page;
    this.loadAll();
  }

  ngOnInit(): void {
    this.loadAll();
  }

  trackId(index: number, item: IEpreuves): number {
    return item.id!;
  }

  delete(epreuves: IEpreuves): void {
    const modalRef = this.modalService.open(EpreuvesDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.epreuves = epreuves;
    // unsubscribe not needed because closed completes on modal close
    modalRef.closed.subscribe(reason => {
      if (reason === 'deleted') {
        this.reset();
      }
    });
  }

  protected sort(): string[] {
    const result = [this.predicate + ',' + (this.ascending ? 'asc' : 'desc')];
    if (this.predicate !== 'id') {
      result.push('id');
    }
    return result;
  }

  protected paginateEpreuves(data: IEpreuves[] | null, headers: HttpHeaders): void {
    this.links = this.parseLinks.parse(headers.get('link') ?? '');
    if (data) {
      for (const d of data) {
        this.epreuves.push(d);
      }
    }
  }
}
