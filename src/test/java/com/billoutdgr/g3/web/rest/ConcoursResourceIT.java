...
class ConcoursResourceIT {
    ...
    void createConcours() throws Exception {
        ...
        // Create the Concours
        ConcoursDTO concoursDTO = concoursMapper.toDto(concours);
        restConcoursMockMvc
            .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(concoursDTO)))
    ...
    void putNewConcours() throws Exception {
        ...
        ConcoursDTO concoursDTO = concoursMapper.toDto(updatedConcours);

        restConcoursMockMvc
            .perform(
                put(ENTITY_API_URL_ID, concoursDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(concoursDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamConcours() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restConcoursMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(concoursDTO)))
    ...
    void patchNonExistingConcours() throws Exception {
        ...
        restConcoursMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, concoursDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(concoursDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamConcours() throws Exception {
        ...
        restConcoursMockMvc
            .perform(
                patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(TestUtil.convertObjectToJsonBytes(concoursDTO))
            )
            .andExpect(status().isMethodNotAllowed());
