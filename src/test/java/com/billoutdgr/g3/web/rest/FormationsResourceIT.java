...
class FormationsResourceIT {
    ...
    void createFormations() throws Exception {
        ...
        FormationsDTO formationsDTO = formationsMapper.toDto(formations);
        restFormationsMockMvc
            .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(formationsDTO)))
    ...
    void putNewFormations() throws Exception {
        ...
        FormationsDTO formationsDTO = formationsMapper.toDto(updatedFormations);

        restFormationsMockMvc
            .perform(
                put(ENTITY_API_URL_ID, formationsDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(formationsDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamFormations() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restFormationsMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(formationsDTO)))
    ...
    void patchNonExistingFormations() throws Exception {
        ...
        restFormationsMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, formationsDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(formationsDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamFormations() throws Exception {
        ...
        restFormationsMockMvc
            .perform(
                patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(TestUtil.convertObjectToJsonBytes(formationsDTO))
            )
            .andExpect(status().isMethodNotAllowed());
